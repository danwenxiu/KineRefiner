...
class TransBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
        ...
        if mixer_type == 'crossattention':
            self.local_attention_list = nn.ModuleList([
                Attention(dim, dim, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop=drop, mode=mode) for i in
                range(3)
            ])
            ...
            self.self_attention = Attention(dim, dim, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop=drop,
                                            mode=mode, vis='yes')
    ...
    def forward(self, x):  # x=(4,243,17,128)
        """
        x: tensor with shape [B, T, J, C]
        """

        if self.mixer_type == 'crossattention':  ##x=（4,243,17,128）
            x = self.forward_local(x)
            self.len = x.shape[1] // 3
            x = self.forward_cross(x, self.len)
            return x
        if self.use_layer_scale:  # x=(4,243,17,128)
            x = x + self.drop_path(  # x=(4,243,17,128)
                self.layer_scale_1.unsqueeze(0).unsqueeze(0)
                * self.mixer(self.norm1(x)))
            x = x + self.drop_path(
                self.layer_scale_2.unsqueeze(0).unsqueeze(0)  # x=(4,243,17,128)
                * self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.mixer(self.norm1(x)))
            x = x + self.drop_path(self.mlp(self.norm2(x)))  ##x=（4,243,17,128）
        return x
        ...
        for i in range(3):
            if self.use_layer_scale:
                q[i] = q[i] + self.drop_path(
                    self.layer_scale_1.unsqueeze(0).unsqueeze(0) * self.mixer[i](self.normq(q[i]), self.normkv(kv[i])))
                q[i] = q[i] + self.drop_path(
...
class DSTFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
        ...
        if self.use_adaptive_fusion:
            self.fusion = nn.Linear(dim * 2, 2)  # 经过这个的输出是2
            self._init_fusion()
    ...
    def forward(self, x):
        ...
        x_attn = self.att_temporal(self.att_spatial(x))  # x_attn=(4,243,17,128)
        x_graph = self.graph_temporal(self.graph_spatial(x))  # x_graph=(4,243,17,128)

        alpha = torch.cat((x_attn, x_graph), dim=-1)
        alpha = self.fusion(alpha)  # x=(4,243,17,128)
        alpha = alpha.softmax(dim=-1)  # x=(4,243,17,2)
        x = x_attn * alpha[..., 0:1] + x_graph * alpha[..., 1:2]  # x=(4,243,17,128)

        return x  # x=(4,243,17,128)
...
class MemoryInducedBlock(nn.Module):
    ...
    def forward(self, x, pose_query):  # x=(4,243,17,128)    pose_query=（4,81,17,128）
        """
        x: tensor with shape [B, T, J, C]
        """
        x = list(torch.chunk(x, 3, dim=1))  # x=(4,81,17,128)

        for i in range(3):
            x[i] = x[i] + self.drop_path(
                self.layer_scale[i].unsqueeze(0).unsqueeze(0) * self.local_attention_list[i](self.local_norms[i](x[i])))
            x[i] = x[i] + self.drop_path(
                self.layer_scale[i + 3].unsqueeze(0).unsqueeze(0) * self.loacl_mlps[i](self.local_norms[i + 3](x[i])))

        x = torch.cat(x, dim=1)

        x, pose_query = self.cross_temporal(x, pose_query)  # x=(4,243,17,128)     pose_query=（4,81,17,128）
